...
for data in input_data:
	...
	page = urllib.urlopen(data['id']).read() 
	paragraphs = justext.justext(page, justext.get_stoplist('English')) 

	final_text = ""
	for paragraph in paragraphs: 
		if paragraph['class'] == 'good': 
			final_text += paragraph['text'].encode('utf-8')
	
	processed_data = re.sub( '\s+', ' ', final_text).strip()
...
with open('./output_readability_txt/processed_data_justext_'+str(int(time.time()))+'.json', 'w') as outfile:
